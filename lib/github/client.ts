...
export interface RawContributionRepo extends RawRepo {
...
export interface RawPayload {
  ...
  recentCommitRepos: RawContributionRepo[]; // public repos recently committed to, including org repos
    ...
    query Profile($login: String!) {
      user(login: $login) {
        ...
        repositories(ownerAffiliations: OWNER, isFork: false, first: 100, orderBy: { field: STARGAZERS, direction: DESC }) {
          totalCount
          nodes { name description url stargazerCount primaryLanguage { name } createdAt pushedAt }
        }
...
function normalize(user: UserNode, lifetimeContributions: number): RawPayload {
  ...
  const recentCommitRepos: RawContributionRepo[] = user.recent.commitContributionsByRepository
    ...
    .map((entry) => ({
      ...
      createdAt: entry.repository.createdAt,
