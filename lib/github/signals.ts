...
const MIN_COMMITS_FOR_LANGUAGE_REPO = 2;
const yearOf = (iso: string) => (iso ? new Date(iso).getUTCFullYear() : null);

function languageRepos(p: RawPayload) {
  const byUrl = new Map<string, { language: string | null }>();
  for (const repo of p.repos) byUrl.set(repo.url, repo);
  for (const repo of p.recentCommitRepos) {
    if (repo.recentCommits >= MIN_COMMITS_FOR_LANGUAGE_REPO) byUrl.set(repo.url, repo);
  }
  return [...byUrl.values()];
}
...
export function signalsFromPayload(p: RawPayload, now = Date.now()): Signals {
  ...
  const reposForLanguages = languageRepos(p);
  const langs = new Set(reposForLanguages.map((r) => r.language).filter(Boolean) as string[]);
  ...
  const rankedLanguages = rankLanguages(reposForLanguages);
  ...
  return {
    ...
    recent_spike,
    topRepositories: p.repos.slice(0, 6).map((repo) => ({
      name: repo.name,
