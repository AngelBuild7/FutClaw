// Card-image capture. The card's signature (futclaw.com + @handle) is hidden on
...
// `.futclaw-capturing`, which reveals the signature.
//
...
// Class added to the capture clone; `.futclaw-signature` reveals under it.
export const SIGNATURE_CLASS = "futclaw-capturing";
...
export async function renderCardImage<T>(
  ...
  if (opts.transparent) {
    ...
    clone.querySelectorAll<HTMLElement>(".futclaw-card-frame").forEach((el) => {
