...
import { resolveCardTheme } from "@/components/card/finishTheme";
import { languageLogoUrl } from "@/lib/github/languages";
import { loadCardFonts } from "./card";
import { CARD_IMAGE_WIDTHS, cardImageHeight, type CardImageSize } from "@/lib/cardImageSize";
...
// The standalone embeddable card image: futclaw.com/<user>.png.
export async function renderCardImage(card: Card, size: CardImageSize = "md"): Promise<ImageResponse> {
  const width = CARD_IMAGE_WIDTHS[size];
  const assets = await loadCardAssets(card, width);
  return new ImageResponse(cardTree(card, assets, width), {
    width,
    height: cardImageHeight(width),
    fonts: assets.fonts,
