...
const healthKey = (idx: number) => `futclaw:ghtoken:v1:${idx}`;
...
const USER = {
  ...
  bio: "Builds small tools.",
  websiteUrl: "https://example.dev",
  url: `https://github.com/${LOGIN}`,
  twitterUsername: "someuser_dev",
  location: null,
...
describe("fetchProfile token pool", () => {
  it("shards a scout onto its hash-assigned token and threads it through every query", async () => {
    ...
    expect(payload.bio).toBe("Builds small tools.");
    expect(payload.websiteUrl).toBe("https://example.dev");
    expect(payload.url).toBe(`https://github.com/${LOGIN}`);
    expect(payload.twitterUsername).toBe("someuser_dev");
