...
function PlayerCard({ card }: { card: Card }) {
  ...
  const identityLogo =
    card.platform === "youtube"
      ? card.categoryLogo
        ? { src: categoryLogoUrl(card.categoryLogo.slug), name: card.categoryLogo.name }
        : null
      : card.languageLogo
        ? { src: languageLogoUrl(card.languageLogo.slug), name: card.languageLogo.name }
        : null;
  ...
  return (
    <div className="futclaw-card-frame" style={wrap}>
      {/* tier background art */}
      <img
        src={t.bg}
        alt=""
      ...
      {/* top language/category logo — a transparent PNG in the left identity column (under
      ...
      {identityLogo && (
        <img
          src={identityLogo.src}
          crossOrigin="anonymous"
          onError={hideOnError}
          alt={identityLogo.name}
          title={identityLogo.name}
          style={{
      ...
      {/* signature — maker's mark (bottom-left) + handle (bottom-right) on the
          ...
          clone (CardActions tags it `.futclaw-capturing`), so it lands in every
          download / copy / share image but never on screen. See lib/capture.ts
          + the `.futclaw-signature` rule in globals. */}
      <div className="futclaw-signature">
        <div
          style={{
            ...at(8, 94.8),
            fontFamily: FONT_BOLD,
