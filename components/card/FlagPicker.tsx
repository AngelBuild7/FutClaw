    ...
    <div ref={rootRef} className="pointer-events-none absolute inset-0 z-20">
      ...
      {open && (
        <div
          ...
          // Prefer below the flag slot, but flip upward on short viewports so it
          // doesn't collide with the share actions below the card.
          style={
            openUp
              ? { left: SLOT.left, bottom: `calc(100% - ${SLOT.top} + 2.5%)` }
              : { left: SLOT.left, top: `calc(${SLOT.top} + ${SLOT.height} + 2.5%)` }
          }
        >
