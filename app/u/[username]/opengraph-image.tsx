...
import { resolveResultTheme } from "@/components/card/finishTheme";
import type { Card } from "@/lib/scoring/types";

export const runtime = "nodejs";
export const alt = "FutClaw player card";
...
export default async function Image({
    ...
    return new ImageResponse(
      (
        ...
        >
          ...
          <div style={{ display: "flex", fontSize: 30, color: "#39d353", fontWeight: 700, marginTop: 26 }}>futclaw.com</div>
        </div>
      ),
  ...
  const card = applyCardOverrides(raw, overrides, pickFlag);
  const accent = card.customization ? resolveResultTheme(card).ink : (card.founder?.accent ?? TIER_ACCENT[card.finish] ?? "#39d353");
    ...
    (
      ...
      >
        ...
        <div style={{ display: "flex", flexDirection: "column", flex: 1 }}>
          ...
          <div style={{ display: "flex", fontSize: 26, color: "#6e7681", marginTop: 32 }}>futclaw.com/{card.login}</div>
        </div>
      </div>
    ),
